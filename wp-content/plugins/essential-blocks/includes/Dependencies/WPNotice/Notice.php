...
class Notice extends Base {
	...
	/**
	 * @var array $options<string|int|bool|null>
	 */
	...
	public function __construct( ...$args ) {
		...
		if ( ! isset( $queue[ $id ] ) || ( ! empty( $this->options['refresh'] ) && ( empty( $queue[ $id ]['refresh'] ) || $queue[ $id ]['refresh'] != $this->options['refresh'] ) ) ) {
			$queue[ $id ]   = array();
			$_eligible_keys = array( 'start', 'expire', 'recurrence', 'refresh' );
			array_walk(
				$options,
				function ( $value, $key ) use ( $id, &$queue, $_eligible_keys ) {
		...
		if ( ! empty( $this->options['do_action'] ) ) {
			$this->do_action();
		}
	}
	...
	public function display( $force = false ) {
		...
		printf(
			...
			! empty( $content['thumbnail'] ) ? wp_kses_post($this->get_thumbnail( $content['thumbnail'] )) : '',
			! empty( $content['html'] ) ? wp_kses_post($content['html']) : $content,
			! empty( $links ) ? wp_kses_post($this->links( $links )) : ''
		);
	}

	public function get_links() {
		return ! empty( $this->content['links'] ) ? $this->content['links'] : ( ! empty( $this->options['links'] ) ? $this->options['links'] : array() );
	}
	...
	public function attributes( $params = array() ) {
		...
		$classname = '';
		if ( isset( $params['data-dismiss'] ) && $params['data-dismiss'] ) {
	...
	public function get_classes() {
		 $classes = array( 'wpnotice-wrapper notice', $this->app->app );
